// JWS-GEE: Algoritmo para generar alertas tempranas de todo el ecuador (procesamiento por zonas) considerando la orbita ascendente (Sentinel-1).
// Fecha desarrollo: 08/05/2025
// Dev: Ing. Jimmy Wladimir Cabrera Soto
// Code: app_sata_orbit_asc.js
...
function processInBatches(data, batchSize) {
  ...
  for (var i = 0; i < keys.length; i += batchSize) { // var i = 0; i < keys.length; i += batchSize
      ...
      batch.forEach(function(key) {
            ...
            Map.addLayer(desc_date1, {min: -25, max: 5}, 'Órbita ascendente Periodo 1 (Cuad: '+ data[key].name + ')', 0);
            Map.addLayer(desc_date2, {min: -25, max: 5}, 'Órbita ascendente Periodo 2 (Cuad: '+ data[key].name + ')', 0);
            Map.addLayer(asc_desc, {min: -3, max: 20, gamma: 0.4, contrast:10}, 'RGB Sentinel 1 Ascendente (Cuad: '+ data[key].name + ')', 1);
            ...
            Map.addLayer(cambios, {palette:"red"}, 'Cambios_fecha_DN_VH_Asc_XXXX');
            ...
            Export.table.toDrive({
                ...
                description: 'fechas_asc_'+ data[key].name + '_' +fecha4,
                fileFormat: "CSV",
                selectors: ['dn','date']
            });
            ...
            Export.image.toDrive({
                ...
                description: 'cambios_bosques_VH_asc_'+ data[key].name + '_' +fecha4,
                scale: 10,
                region: table,
                fileFormat: 'GeoTIFF',
                maxPixels: 1E13
            ...
            Export.image.toDrive({
                ...
                description: 'cambios_manglares_VH_asc_'+ data[key].name + '_' +fecha4,
                scale: 10,
                region: table,
                fileFormat: 'GeoTIFF',
                maxPixels: 1E13,
            ...
            /*Export.image.toDrive({
                image: asc_desc,
                description: 'RGB_Sentinel_1_Ascendente',
                scale: 10,
                maxPixels:1e13,
                region: table,
            });*/
