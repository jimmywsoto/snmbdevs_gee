...
// Fecha desarrollo: 31/07/2025
// Fecha actualización: 31/07/2025
// Dev: Ing. Jimmy Wladimir Cabrera Soto
// Code: app_sata_one_orbit_v1.js
// Legacy: APP_SATA_1O (Dev: Fabricio Garcés)
// =====================================================================================

/* ======================= PARÁMETROS DE ANÁLISIS ======================= */
// 1.- Imports
var zona = ee.FeatureCollection("users/ecuadorbfast/SATAASSEST/cuadrantes_monitoreo_V2");
var table = zona.filter(ee.Filter.inList('zona', ['33', '34']));

///--- Selección de fechas de análisis ---///
var fecha1 = '2025-01-31';
var fecha2 = '2025-04-29';
var fecha3 = '2025-04-30';
var fecha4 = '2025-07-31';
...
function processSingle() {
  ...
  var descVH = imgVV.filter(ee.Filter.eq('orbitProperties_pass', 'ASCENDING'));
...
Map.addLayer(desc_date1, {min: -25, max: 5}, 'Órbita ascendente Periodo 1', 0);
Map.addLayer(desc_date2, {min: -25, max: 5}, 'Órbita ascendente Periodo 2', 0);
Map.addLayer(asc_desc, {min: -3, max: 20, gamma: 0.4, contrast:10}, 'RGB Sentinel 1 Ascendente', 1);


///--- Exportar la imagen RGB de Sentinel 1 a Google Drive ---///
Export.image.toDrive({
  image: asc_desc,
  description: 'RGB_Sentinel1_Ascendente',
  scale: 10,
  maxPixels:1e13,
  region: table,
});
...
Map.addLayer(bosque2022,{min:0.0, max:1.0,palette:['229512']},'máscara bosques 2020');
Map.addLayer(manglar2022,{min:0.0, max:1.0,palette:['13ada3']},'máscara manglares 2022');
Map.addLayer(cambiosbosques,{palette:"yellow"},'cambios en bosque 2020');
...
Export.table.toDrive({
  ...
  description: 'fechas_asc_50-51-52_'+fecha4,
  fileFormat: "CSV",
  selectors: ['dn','date']
});

var tableURL = table.getDownloadURL({
    format: 'CSV'
})

print('Descargar tabla:', tableURL);

/*
// Exportar el ráster con las zonas de cambio y valor DN.
Export.image.toDrive({
   image: cambios,
   description: 'cambios_DN_VH_Desc_xxFechaxx_',
   scale: 10,
   region: table,
   fileFormat: 'GeoTIFF',
   maxPixels: 1E13
...
Export.image.toDrive({
   ...
   description: 'cambios_bosques_VH_desc_50-51-52_'+fecha4,
   scale: 10,
   region: table,
   fileFormat: 'GeoTIFF',
   maxPixels: 1E13
...
Export.image.toDrive({
   ...
   description: 'cambios_manglar_asc_50-51-52_'+fecha4,
   scale: 10,
   region: table,
   fileFormat: 'GeoTIFF',
   maxPixels: 1E13
...
var cBosqueURL = cambiosbosques.getDownloadURL({
    description: 'cambios_manglares_VH_desc_50-51-52_',
    scale: 20,
    region: zona4326,
    format: 'GeoTIFF',
    crs: 'EPSG:4326',
...
// Descarga directa (efectivo para exportar por Procesos listados: ZP01, ZP02, etc)
  ...
  print('URL de descarga:', cBosqueURL);
